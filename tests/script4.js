...
//        center: [-94.6859, 46.7296],
  ...
  function queryInitial(feature) {
      const str = feature;
      const query = "";
      query = PolygonsLayer.createQuery();
      return PolygonsLayre.queryFeatures(query);
      
  }
      
// dispaly results in graphics layer
      function displayResults(results) {
          ...
          const numCounties = features2.length;
          counties.addMany(features2);
      ...
      function queryCountiesthatIntersect() {
          ...
          query.geometry = counties.graphics.items[0].geometry;
          query.geometry = counties.graphics.items[0].geometry;
      ...
      function displayResults2(results) {
          counties.removeAll();
          const features3 = results.features.map(function (graphic) {
              graphic.symbol = {
                    type: "simple-fill",
                    style: "solid",
                    color: [240, 248, 255, 0.9],
          ...
          const numCounties = features3.length;
          counties.addMany(features3);
      }
